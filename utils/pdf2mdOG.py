...
def to_markdownOG(
    ...
    *,
    ...
    def consolidate_spans(spans):
        """Consolidate spans that are close to each other into coherent lines."""
        consolidated = []
        if not spans:
            return consolidated
        ...
        for next_span in spans[1:]:
            # horizontal distance between current span end and next span start
            distance = next_span['bbox'].x0 - current_span['bbox'].x1

            # vertical alignment by comparing the y0 positions (top of the bbox)
            vertical_alignment = abs(next_span['bbox'].y0 - current_span['bbox'].y0)

            # if spans are close enough horizontally and aligned vertically, merge them
            if distance < 2 and vertical_alignment < 5:  # not sure about threshold values
                current_span['text'] += next_span['text']
                # expand the current span's bbox to include next span
                current_span['bbox'] = current_span['bbox'] | next_span['bbox']
            else:
                # if not close, push current_span to consolidated and move to next_span
