...
## INSTRUCTIONS
   ...
   - The Markdown text may not have the '#' characters, so you must infer the hierarchy based on the formatting and enumerations, markdown formatting, and pattern of the section types. For example, the top level section level might be indicated by '**'. Use the image to help determine the hierarchy. Sections with the same formatting are at the same level.
...
{PROIR_SCHEMA_OR_TEMPLATE_STRING}
...
EVERY SINGLE unique section base types and items in the ToC MUST be included VERBATIM from the text in the JSON object. Do NOT include section titles, only the base types. Do NOT include anything that is not part of the ToC - all ToC sections and items are most likely associated with a number or enumeration and must be the only entries in the JSON object.

Please create a JSON object that maps each unique hierarchical section and item to a corresponding number of '#' characters, based on its hierarchy, from the following ToC Markdown string:
